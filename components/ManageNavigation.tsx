...
export default function ManageNavigation({
  ...
  return (
    <div className={cn('flex items-center justify-end', className)}>
      <Button onClick={() => handleNavigate('prev')} variant={'ghost'} className='hover:text-blue-800'>
        Trước
      </Button>
      <Button onClick={() => handleNavigate('next')} variant={'ghost'} className='hover:text-blue-800'>
        Sau
      </Button>
    </div>
  );
